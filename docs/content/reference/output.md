...
describes what users get. Pick a format with `-o`, or let cheapshark choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cheapshark <command> -o table   # aligned columns for reading
cheapshark <command> -o jsonl   # one JSON object per line, for piping
cheapshark <command> -o json    # a single JSON array
cheapshark <command> -o csv     # spreadsheet friendly
cheapshark <command> -o tsv     # tab-separated
cheapshark <command> -o url     # just the URL column
cheapshark <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cheapshark <command> --fields id,title,url
```
...
```bash
cheapshark <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cheapshark <command>            # a table, because this is a terminal
cheapshark <command> | wc -l    # JSONL, because this is a pipe
```
