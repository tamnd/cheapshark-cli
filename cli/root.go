// Package cli assembles the cheapshark command tree from the cheapshark
...
// NewApp assembles the kit application from the cheapshark domain. The
...
//
// To add a command, declare it in cheapshark/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := cheapshark.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(cheapshark.Domain{}).Register(app)
